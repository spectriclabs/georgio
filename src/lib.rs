...
const EARTH_RADIUS_METERS: f64 = 6_371_008.8;  // IUGG mean Earth radius
const EPSILON32: f64 = 1.0e-9;

/// Raises a Python ValueError if the coordinates are not valid.
#[pyfunction]
fn valid_coordinates(lon: f64, lat: f64) -> PyResult<()> {
...
#[pyfunction]
fn great_circle_distance_with_radius(lon1: f64, lat1: f64, lon2: f64, lat2: f64, radius: f64) -> PyResult<f64> {
    ...
    let d = f64::atan2(f64::sqrt((cos_lat2 * sin_delta_lon).powi(2) +
             (cos_lat1 * sin_lat2 -
...
/// * `lat2` - The latitude of the end location.
#[pyfunction]
fn great_circle_distance(lon1: f64, lat1: f64, lon2: f64, lat2: f64) -> PyResult<f64> {
...
#[pyfunction]
fn line_of_bearing_with_radius(lon: f64, lat: f64, bearing: f64, distance: f64, radius: f64) -> PyResult<(f64, f64)> {
    valid_coordinates(lon, lat)?;

    let rad_bearing = f64::to_radians(bearing);
    let rad_lat = f64::to_radians(lat);
    let rad_lon = f64::to_radians(lon);
    let d_div_r = distance / radius;
    let rad_new_lat = f64::asin(
        f64::sin(rad_lat)*f64::cos(d_div_r) +
        f64::cos(rad_lat)*f64::sin(d_div_r)*f64::cos(rad_bearing)
    );
    let rad_new_lon = rad_lon + f64::atan2(
        f64::sin(rad_bearing)*f64::sin(d_div_r)*f64::cos(rad_lat),
        f64::cos(d_div_r)-f64::sin(rad_lat)*f64::sin(rad_new_lat)
    );
    let new_lat = f64::to_degrees(rad_new_lat);
    let new_lon = f64::to_degrees(rad_new_lon);
    Ok((new_lon, new_lat))
}
...
/// * `distance` - The distance in meters.
#[pyfunction]
fn line_of_bearing(lon: f64, lat: f64, bearing: f64, distance: f64) -> PyResult<(f64, f64)> {
...
#[pyfunction]
fn bounding_box_for_point(lon: f64, lat: f64, distance_meters: f64) -> PyResult<(f64, f64, f64, f64)> {
    valid_coordinates(lon, lat)?;
    ...
    let delta_lon_rad = f64::asin(f64::sin(dist_radians) / f64::cos(lat_rad));
    ...
    let bbox_north = f64::to_degrees(north_rad);
    let bbox_south = f64::to_degrees(south_rad);
    let bbox_east = f64::to_degrees(east_rad);
    let bbox_west = f64::to_degrees(west_rad);
...
/// * y - Tile y coordinate
/// * z - Tile zoom level
fn _wm_upper_left(x: u32, y: u32, z: u32) -> (f64, f64) {
    // useful link: https://www.maptiler.com/google-maps-coordinates-tile-bounds-projection/
    let inv_z2: f64 = 1.0 / 2.0_f64.powi(z as i32);
    let lon = (x as f64) * inv_z2 * 360.0 - 180.0;
    let lat = f64::to_degrees(f64::atan(f64::sinh(PI * (1.0 - 2.0 * (y as f64) * inv_z2))));
    (lon, lat)
}
...
/// * z - Tile zoom level
#[pyfunction]
fn wm_upper_left(x: u32, y: u32, z:u32) -> PyResult<(f64, f64)> {
...
/// * y - Tile y coordinate
/// * z - Tile zoom level
fn _wm_bounds(x: u32, y: u32, z: u32) -> (f64, f64, f64, f64) {
...
/// * z - Tile zoom level.
#[pyfunction]
fn wm_bounds(x: u32, y: u32, z: u32) -> PyResult<(f64, f64, f64, f64)> {
    Ok(_wm_bounds(x, y, z))
}
...
fn tile_center_lon_lat(west: f64, south: f64, east: f64, north: f64) -> (f64, f64) {
...
#[pyfunction]
fn wm_tile_expanded_bbox(x: u32, y: u32, z: u32, expansion_meters: f64) -> PyResult<(f64, f64, f64, f64)> {
    ...
    let tile_center_lon_rad = f64::to_radians(tile_center_lon);
    let tile_center_lat_rad = f64::to_radians(tile_center_lat);
    let delta_lon_rad = f64::asin(f64::sin(search_radians) / f64::cos(tile_center_lat_rad));
    ...
    let bbox_east = if !delta_lon_rad.is_nan() { f64::to_degrees(east_rad) } else { tile_east };
    let bbox_west = if !delta_lon_rad.is_nan() { f64::to_degrees(west_rad) } else { tile_west };
